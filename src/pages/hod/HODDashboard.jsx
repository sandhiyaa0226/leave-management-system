...
function HODDashboard() {
  ...
  const fetchRequests = async () => {
    try {
      const res = await api.get(`/leave-requests/hod/${user.id}`);
  ...
  const handleReview = async (id, status) => {
    ...
    setSuccess('');
    try {
      await api.put(`/leave-requests/${id}/hod-review`, {
        status,
  ...
  const pendingRequests = requests.filter((r) => r.hod_status === 'pending');
  const reviewedRequests = requests.filter((r) => r.hod_status !== 'pending');

  return (
    <div style={{ maxWidth: '700px', margin: '50px auto' }}>
      <Navbar title="HOD Dashboard" />
      <h2>HOD Dashboard — {user.name}</h2>
