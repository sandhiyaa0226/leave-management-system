...
function PrincipalDashboard() {
  ...
  const fetchRequests = async () => {
    try {
      const res = await api.get(`/leave-requests/principal/${user.id}`);
  ...
  const handleReview = async (id, status) => {
    ...
    setSuccess('');
    try {
      await api.put(`/leave-requests/${id}/principal-review`, {
        status,
        remarks: remarksMap[id] || '',
        approver_id: user.id
      });
      setSuccess(`Request ${status} (final decision)`);
      fetchRequests();
    } catch (err) {
  ...
  const pendingRequests = requests.filter((r) => r.principal_status === 'pending');
  const reviewedRequests = requests.filter((r) => r.principal_status !== 'pending');
  ...
  return (
    <div style={{ maxWidth: '700px', margin: '50px auto' }}>
      ...
      <h3>Pending Final Approval ({pendingRequests.length})</h3>
      {pendingRequests.length === 0 && <p>No pending requests.</p>}
      {pendingRequests.map((r) => (
        <div key={r.id} style={{ border: '1px solid #ccc', padding: '12px', marginBottom: '10px' }}>
          <p><strong>{r.student_name}</strong> ({r.roll_number || 'No Roll No'}, {r.department_name}) — {formatDate(r.from_date)} to {formatDate(r.to_date)} ({r.number_of_days} days)</p>
